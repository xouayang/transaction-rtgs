...
//           <div className="h-[400px] sm:h-[500px] md:h-[600px] lg:h-[700px] xl:h-[800px] 2xl:h-[1000px] 3xl:h-[1500px]  relative rounded-lg border border-gray-200 bg-white shadow-inner overflow-hidden w-full">
//             {/* Fixed Header */}
...
//       if (!isPaused && !scrollCompleteRef.current) {
//         scrollPosition += scrollSpeed;
...
//         if (isAtBottom) {
//           // Mark scrolling as complete and stop
...
//     // Reset scroll position and completion state when tab changes
//     container.scrollTop = 0;
//     scrollPosition = 0;
//     scrollCompleteRef.current = false;
...
//   // Smart tab switching that waits for scroll completion OR 30 seconds
//   useEffect(() => {
//     if (intervalRef.current) {
//       clearInterval(intervalRef.current);
//     }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (!container || !currentData || currentData.length === 0) {
      scrollCompleteRef.current = true; // No data means scrolling is "complete"
      return;
    }
  ...
  // Tab switching every 30 seconds - ALWAYS switch when timer is done
  useEffect(() => {
    if (intervalRef.current) {
      clearInterval(intervalRef.current);
    }
      ...
      <Card className="pb-6">
        <CardBody className="p-6">
          <div className="h-[400px] sm:h-[500px] md:h-[600px] lg:h-[700px] xl:h-[800px] 2xl:h-[1000px] 3xl:h-[1500px] relative rounded-lg border border-gray-200 bg-white shadow-inner overflow-hidden w-full">
            ...
            <div
              ref={tableBodyRef}
              className="h-[calc(100%-55px)] overflow-hidden relative w-full"
            >
              {isLoading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-gray-500">ກຳລັງໂຫຼດ...</div>
